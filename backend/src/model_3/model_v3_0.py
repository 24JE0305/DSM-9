...
#
# Attribute names match Colab notebook exactly so saved
# weights load without key mismatch errors.
# ============================================================
...
class PositionalEncoding(nn.Module):
    def __init__(self, d, max_len=512, drop=0.1):
        ...
        self.register_buffer("pe", pe.unsqueeze(0))   # saves as "pe.pe" ✅
...
class TransformerBlock(nn.Module):
    def __init__(self, d, heads=4, ff=256, drop=0.1):
        ...
        self.n1   = nn.LayerNorm(d)    # saves as "transformer.X.n1" ✅
        self.n2   = nn.LayerNorm(d)    # saves as "transformer.X.n2" ✅
...
class TemporalAttention(nn.Module):
    def __init__(self, h):
        super().__init__()
        self.a = nn.Linear(h, 1)       # saves as "bilstm.attn.a" ✅
...
class DSM9_v3(nn.Module):
    def __init__(self, n_feat, d=64, heads=4, n_trans=2,
        ...
        self.proj        = nn.Linear(n_feat, d)              # saves as "proj" ✅
        self.pe          = PositionalEncoding(d, drop=drop)  # saves as "pe.pe" ✅
        self.transformer = nn.Sequential(
            *[TransformerBlock(d, heads, d * 4, drop) for _ in range(n_trans)]
        )
        self.bilstm = BiLSTMAttn(n_feat, lstm_h, lstm_l, drop)
        self.fusion = nn.Sequential(                         # saves as "fusion.X" ✅
